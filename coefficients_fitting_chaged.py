...
#TODO
def autofit(x:pd.DataFrame, y:ND, device,num_epochs=50,batch_size=32,lr=0.1,use_net = True):
    '''
    系数拟合(只考虑一维情况)
    '''
    # 将pd数据转为np数组
...
def fit(x:pd.DataFrame, y:ND,device):
    ...
    for i in range(X.shape[1]):
        ...
        # r2, p, loss = analytical_solving(X[:, i], y[:])
        # 神经网络
        r2, p, loss = net_solving(X[:, i], y[:],num_epochs=2,batch_size=64,lr=1e-4)
...
def net_solving(X:torch.tensor, y:torch.tensor, num_epochs=40, batch_size=100, lr=0.1):
    ...
    # 获取模型参数
    weight = net[0].weight.data.cpu().numpy()[0, 0]
    bias = net[0].bias.data.cpu().numpy()[0]

    # 计算R2（直接用模型预测）
    with torch.no_grad():
        pred = net(features)
    r2 = get_r2(labels, pred)
